...
const CheckBox = ({ label, selected = false, onChange = () => { } }) => {
    return (
        <label className="flex gap-3 items-center cursor-pointer mt-2 text-sm">
            <input type="checkbox" selected={selected} onChange={(e) => onChange(e.target.selected, label)} className="w-4 h-4" />
...
const RadioButton = ({ label, selected = false, onChange = () => { } }) => {
    return (
        <label className="flex gap-3 items-center cursor-pointer mt-2 text-sm">
            <input type="radio" name="sortOption" selected={selected} onChange={onChange(label)} className="w-4 h-4" />
            <span className="font-light select-none">{label}</span>
        </label>
    )
};
